...
T SLinkedList<T>::removeAt(int index)
{
    ...
    else if (index == count - 1)
    {
        ...
        Node *prev = head;
        Node *curr = head->next;
        while (curr->next != tail) // traverse to the last node
        {
            curr = curr->next;
            prev = prev->next;
        }
        prev->next = tail;   // Connect the prev of CurrNode to the tail'
        tail->next = prev;   // set the tail->next to the new LastNode;
        answer = curr->data; // get the data
        delete curr;         // delete the old lastNode
    }
    else
    {
        Node *prev = head;              // get the first internal node
        Node *curr = prev->next;        //get the second internal node
        for (int i = 0; i < index; i++) // loop until the current node is the node that we need to del
        {
            prev = prev->next;
            curr = curr->next;
        }
...
bool SLinkedList<T>::removeItem(T item, void (*removeItemData)(T))
{
    ...
    while(current!=tail)
    {
        found = equals(current->data, item, this->itemEqual);
        if(found == true)
        {
